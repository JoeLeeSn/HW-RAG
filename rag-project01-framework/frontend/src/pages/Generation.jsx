...
const Generation = () => {
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-6 text-gray-800">Generation</h2>
        ...
        <div className="col-span-4 space-y-4">
          <div className="p-4 border rounded-lg bg-white shadow-sm">
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1 text-gray-700">Search Results File</label>
                <select
                  value={selectedFile}
                  onChange={(e) => setSelectedFile(e.target.value)}
                  className="block w-full p-2 border rounded min-w-[300px]"
                >
                  ...
                  {(provider === 'openai' || provider === 'deepseek') && (
                    <div>
                      ...
                      <input
                        ...
                        className="block w-full p-2 border rounded bg-white text-gray-800 border-gray-300 
        ...
        <div className="col-span-8">
          {selectedFile ? (
            <>
              ...
              <div className="mb-6 p-4 border rounded-lg bg-white shadow-sm">
                ...
                <div className="space-y-4 max-h-[300px] overflow-y-auto">
                  {searchResults.map((result, idx) => (
                    <div key={idx} className="p-4 border rounded bg-gray-50 hover:bg-blue-50 transition-colors">
                      <div className="flex justify-between items-start mb-2">
                        <span className="font-medium text-sm text-gray-700 bg-blue-100 px-2 py-1 rounded-full">
                          Match Score: {(result.score * 100).toFixed(1)}%
                        </span>
                        <span className="text-xs text-gray-600">
                          {result.metadata.source} (p.{result.metadata.page})
                        </span>
                      </div>
                      <p className="text-sm text-gray-800 whitespace-pre-wrap mt-2 leading-relaxed">
                        {result.text}
                      </p>
                    </div>
                  ))}
              ...
              {response && (
                <div className="p-4 border rounded-lg bg-white shadow-sm">
                  <h3 className="text-xl font-semibold mb-4 text-gray-800 border-b pb-2">Generated Response</h3>
                  <div className="p-4 border rounded bg-white prose max-w-none">
